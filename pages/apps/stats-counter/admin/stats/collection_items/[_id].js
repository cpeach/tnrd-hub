...
export default function CollectionItems(props) { 
	...
	useEffect(async () => {
		let isMounted = true;
		let _items = await client({url:'/stats-counter/collection_items/list',params:{method:'POST',body:{filters:{collection:_id}}}});
	...
	if(items&&item){
		l_data.list.columns[1].render = (p)=>{return <Link href={"/stats-counter/admin/groups/collection_items/edit/"+p}>Edit</Link>}
