...
export default function Reports(props) { 
	...
	useEffect(async () => {
		let isMounted = true;
		let _reports = await client({url:'/stats-counter/reports/items/list',params:{method:"POST",body:{}}});
		console.log(reports)
		if(isMounted){
			setReports(_reports)
		}
	...
	if(reports){
		...
		l_data.list.columns[4].render = (p)=>{return <Link href={"/stats-counter/admin/reporting/"+p}>Edit</Link>}
