...
export default function TNRD(props) { 
	...
	useEffect(async () => {
		let isMounted = true;
		let _items = await client({url:'/incident-reports/incidents/tnrd/list',params:{method:'POST',body:{}}});
	...
	if(items&&settings&&administrators){
		...
		l_data.list.columns[2].render = (p)=>{return settings["general-types"][p].name}
		l_data.list.columns[3].render = (p)=>{return settings["departments"][p].name}
		l_data.list.columns[4].render = (p)=>{return <Link href={"/incident-reports/admin/incidents/tnrd/details/"+p}>View</Link>}
		l_data.list.columns[5].render = (p)=>{return <Link href={"/incident-reports/admin/incidents/tnrd/edit/"+p}>Edit</Link>}
