...
export default function Update(props){
	...
	useEffect(async () => {
		...
		let _types = await client({url:'/tnrl-stats/collections/types'});
	...
	const handleSubmit = async(data) => {
		...
		let params = {years:data.years,months:data.months,types:data.types}
		...
		var results = await client({url:"/tnrl-stats/reports/collections/circ",params:{method:"PUT",body:data}})
		success(["Success","A new Report record was updated."]);
		window.location.href = '/tnrl-stats/admin/reports/collections/usage'; 
	}
	...
	if(types&&report){
		...
		data.form.title = "Update Collection Report";
		...
		data.form.fields[4].options = types.map((item,i)=>({label:item.name,name:item.type_id,value:item.type_id}));
		data.form.fields[4].value = report.parameters.types
