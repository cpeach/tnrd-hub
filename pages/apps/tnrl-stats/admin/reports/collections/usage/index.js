...
export default function Reports(props) { 
	...
	useEffect(async () => {
		let isMounted = true;
		let _reports = await client({url:'/tnrl-stats/reports/collections/circ'});
		if(isMounted){
			setReports(_reports)
		}
	...
	if(reports){
		...
		l_data.list.columns[3].render = (p)=>{return <Link href={p?p:""}>{"Download"}</Link>}
		l_data.list.columns[4].render = (p)=>{return <Link href={"/tnrl-stats/admin/reports/collections/usage/"+p}>Edit</Link>}
