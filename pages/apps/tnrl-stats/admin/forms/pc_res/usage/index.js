...
export default function Update(props){
	...
	const handleSubmit = async(data) => {
		...
		var results = await client({url:"/tnrl-stats/pc_res/usage/",params:{method:"POST",body:data}});
		success(["Success","Usage stats were updated."]);
		  
		window.location.href = '/tnrl-stats/admin/forms/pc_res/'  
	...
	useEffect(async () => {
		...
		let _items = await client({url:"/tnrl-stats/pc_res/locations"});
	...
	if(items){
		...
		items.map((item,i)=>{
			form.fields.push({"tag":"text","label":item.name,"required":false,"container":0,"size":12,"attributes":{"name":item.lib_id}})
		})
