...
export default function Update(props){
	...
	const handleSubmit = async(data) => {
		...
		var results = await client({url:"/tnrl-stats/circs/ml",params:{method:"POST",body:data}});
		success(["Success","Circ stats were updated."]);
		  
		window.location.href = '/tnrl-stats/admin/forms/circs/'  
	...
	useEffect(async () => {
		...
		let _items = await client({url:"/tnrl-stats/circs/ml/locations"});
	...
	if(items){
		...
		form.fields[2].options = items.map((item,i)=>({label:item.stopname,name:item.ml_id,value:item.ml_id}));
