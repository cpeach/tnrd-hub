...
export default function Update(props){
	...
	const handleSubmit = async(data) => {
		...
		var results = await client({url:"/tnrl-stats/wifi/usage/bulk",params:{method:"POST",body:data}});
	...
	if(items){
		...
		items.map((item,i)=>{
			form.fields.push({"tag":"text","label":item.name+" Usage","required":false,"container":0,"size":12,"attributes":{"name":item.lib_id}})
		})
