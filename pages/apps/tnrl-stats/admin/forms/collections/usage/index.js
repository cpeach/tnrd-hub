...
export default function Update(props){
	...
	const handleSubmit = async(data) => {
		...
		var results = await client({url:"/tnrl-stats/collections/circ/",params:{method:"POST",body:data}});
		success(["Success","Usage stats were updated."]);
		  
		window.location.href = '/tnrl-stats/admin/forms/collections/'  
	...
	useEffect(async () => {
		...
		let _items = await client({url:"/tnrl-stats/collections/types"});
	...
	if(items){
		...
		items.map((item,i)=>{
			form.fields.push({"tag":"text","label":item.name,"required":false,"container":1,"size":4,"attributes":{"name":item.type_id}})
		})
