...
export default function Insert(props){
	...
	const handleSubmit = async(data) => {
		var results = await client({url:"/hub-console/resource_links",params:{method:"PUT",body:data}})
		success(["Success","Your Resource link record was updated."]);
		window.location.href = '/applications/'+application._id; 
	}
	const handleDelete = async(data) => {
		var results = await client({url:"/hub-console/resource_links/"+_id,params:{method:"Delete"}})
		success(["Success","Your Resource link record was deleted."]);
		window.location.href = '/applications/'+application._id; 
	}
	useEffect(async () => {
		let isMounted = true;
		...
		let _link = await client({url:"/hub-console/resource_links/"+_id});
	...
	if(types && link && application){
		...
		data.form.fields[2].value  = link.url;
		data.form.fields[3].value  = link.description;		
